# -*- coding: utf-8 -*-
"""Formats for imageio to handle MICS Resources
"""
...
from imageio import formats, mimread, volread
...
dummy_format = DummyFormat(
    "mydummy",  # short name
    "An example format that does nothing.",  # one line descr.
    ".foobar .nonexistentext",  # list of extensions
    "iIvV",  # modes, characters in iIvV
)
formats.add_format(dummy_format)
import IPython as ip

root = Path(
    '/home/rikisa/datasets/318_OvCa/'
)

well = root / 'C - 2(fld 1'
img = well / '000_DAPIf__16bit_DF_FF_C - 2(fld 1 wv DAPI - DAPI).tif'

img_list = list(well.glob('0*.[tT][iI]*'))

mims = mimread(img, format='mydummy')
vim = volread(path, format='mydummy')
ip.embed()
